...
$\sum K_e$ = sum of minor loss coefficients for the whole system. This is also set initially, it is usually 2.  
...
Therefore, the total head loss is a function of flow, and is shown in the following equation. Blue will be used to reference _actual_ head loss from now on.
...
The next steps are algebraic rearrangements to solve for $L$. This $L$ describes the _minimum_ length of dosing tube necessary to meet our error constraint at _maximum_ flow. Thus, we will refer to it as $L_{\Pi_{Error}, \, Min}$.
...
$$L_{\Pi_{Error}, \, Min} = L = \left( \frac{1 - \Pi_{Error}}{\Pi_{Error}} \right) \frac{Q_{Max}}{16 \nu \pi} \sum{K_e} $$

Note that this equation is independent of head loss.   
Unfortunately, both $L_{\Pi_{Error}, \, Min}$ and $Q_{Max}$ are unknowns. We can plug this equation for $L_{\Pi_{Error}, \, Min}$ back into the head loss equation at maximum flow, which is $h_{L_{Max}} = \left( \frac{128\nu L Q_{Max}}{g \pi D^4} + \frac{8Q_{Max}^2}{g \pi ^2 D^4} \sum{K_e} \right)$ and rearrange for $Q_{Max}$ to get:
...
The only other equation strictly necessary to design a CDC system is one for the length of the dosing tube(s), $\color{purple}{L_{Min}}$. This is rather simple. First, consider the head loss at maximum flow that was used to get the equation for $Q_{Max}$:

$$ h_{L_{Max}} = \left( \frac{128 \nu L{Q_{Max}}}{g \pi D^4} + \frac{8 Q_{Max}^2}{g \pi^2 D^4} \sum{K_e} \right)$$

Now that we know all of the parameters in this equation except for $\color{purple}{L_{Min}}$, we can solve the equation for $L$. This the _shortest_ tube that generates our required head loss, $h_{L_{Max}}$.

$$\color{purple}{
L_{Min} = \left( \frac{g h_{L_{Max}} \pi D^4}{128 \nu Q_{Max}} - \frac{Q_{Max}}{16 \pi \nu} \sum{K_e} \right)
}$$
**Functions in aide_design:**   
`cdc._length_cdc_tube_array(FlowPlant, ConcDoseMax, ConcStock, DiamTubeAvail, HeadlossCDC, temp, en_chem, KMinor)` Returns $\color{purple}{L_{Min}}$, take in the _plant flow rate_.  
`cdc._len_tube(Flow, Diam, HeadLoss, conc_chem, temp, en_chem, KMinor)` Returns $\color{purple}{L_{Min}}$, takes in the _max flow rate through the dosing tube(s)_.

If you decrease the flow $Q$, $\color{purple}{L_{Min}}$ becomes larger. This is good, it means that this equation for $\color{purple}{L_{Min}}$ works at every flow rate that the CDC will experience.
